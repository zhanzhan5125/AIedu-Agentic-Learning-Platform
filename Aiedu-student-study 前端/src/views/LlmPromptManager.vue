  ...
  <div class="total-container">
    ...
    <el-main style="">
      ...
      <div class="title-container">
        <span class="title">智能批阅</span>
        <router-link type="primary" class="router" to="/admin_home">返回</router-link>
      </div>
        ...
        <div class="prompt-box">
          <div class="prompt-left">
            <div class="mode-selector"
                 style="width:100%;height:40px;justify-content: center;align-items: center;display: flex;padding-top:10px">
              <el-switch
                  style="zoom: 1.1"
                  v-model="preview"
                  active-text="预览模式"
            ...
            <div v-for="(prompt,index) in promptList" :key="index">
              <div class="prompt-grid" :class="{ 'activeGrid': activeGrid === index }">
                <div class="prompt-grid-content clickable" style="display:flex;" @click="selectPrompt(index)">
                  <div class="prompt-grid-left" style="width: 60%;text-align: center">
                    <p style="line-height: 40px">{{ prompt.name }}</p>
                  </div>
                  <div class="prompt-grid-middle" style="width:10%;display: flex; justify-content: center;align-items: center">
                  ...
                  <div class="prompt-grid-right"
                    ...
                    <el-dropdown trigger="click">
                      ...
                      <el-dropdown-menu slot="dropdown">
                        <el-dropdown-item ><el-button style="border:none" @click="changePrompt()">设为启用</el-button></el-dropdown-item>
                      </el-dropdown-menu>
                    </el-dropdown>
                  </div>
                </div>
          ...
          <div class="prompt-middle">
            <div v-if="promptList.length > 0 && promptList[activeGrid] && promptList[activeGrid].prompt"
                 v-for="(item,index) in promptList[activeGrid].prompt" :key="index">
              <div class="text-grid">
...
<!--              </el-popconfirm>-->
    ...
    <!--      <div class="log-box">-->
    <!--        <div class="log-upper" style="width: 100%;height: 50px;border-bottom: solid 1px rgb(200,200,201)">-->
    <!--          <div class="log-title" style="width:14%;text-align: center">-->
    <!--            <span style="line-height: 50px;font-size:20px;font-weight: bold">批阅记录</span>-->
    <!--          </div>-->
    <!--        </div>-->
...
export default {
  ...
  created() {
    apiV1.get('/prompts').then(res => {
      ...
      for (let i = 0; i < promptList.length; i++) {
        let dictionary = {
          name: promptList[i].name,purpose:promptList[i].purpose,ptId:promptList[i].id,
          prompt:parseStringToDictList(promptList[i].content||''),ptStatus:promptList[i].active
        }
  ...
  methods: {
    ...
    changePrompt(){
      apiV1.put(`/prompts/${this.promptList[this.activeGrid].ptId}/activate`).then(response => {
        console.log(response);
        this.$message.success("更改提示词成功")
        this.load()
      })
    },
    load(){
