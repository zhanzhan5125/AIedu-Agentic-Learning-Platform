...
export default {
  data(){return{activeTab:'assignment',keywords:'',selectedChapterIds:[],count:5,difficulty:2,submitting:false,pendingDraft:null,run:{},steps:[],timer:null,drafts:[],selectedDraft:null,draftForm:null,draftListLoading:false,draftLoading:false,draftEditing:false,draftSaving:false,publishDialogVisible:false,publishRange:[],publishSaving:false,publishPickerOptions:{disabledDate(time){return time.getTime()<Date.now()-86400000}},courseMap:null,publishedCourseMap:null,mapGenerating:false,mapSaving:false,editing:false,selectedNode:null,chart:null}},
  computed:{
  ...
  methods:{
    ...
    async generateAssignment(){if(!this.offeringId)return this.$message.warning('请先选择课程');const chapters=this.publishedChapters.filter(item=>this.selectedChapterIds.includes(item.id)).map(item=>item.name);const words=this.keywords.split(/[，,\s]+/).filter(Boolean);this.pendingDraft={title:chapters.join('、')||words.join('、')||'新作业草稿',questionCount:this.count,difficulty:this.difficulty,status:'queued',runId:null,error:null};this.submitting=true;try{const key=`draft-${this.offeringId}-${Date.now()}`;const res=await apiV1.post(`/teacher/offerings/${this.offeringId}/assignment-drafts`,{keywords:words,course_map_node_ids:this.selectedChapterIds,question_count:this.count,difficulty:this.difficulty,question_kinds:['short_answer'],knowledge_point_ids:[],idempotency_key:key});this.pendingDraft={...this.pendingDraft,runId:res.data.agent_run_id,status:res.data.status};this.persistPendingDraft();this.pollAssignment(res.data.agent_run_id)}catch(error){this.pendingDraft={...this.pendingDraft,status:'failed',error:this.errorMessage(error)||'智能出题任务创建失败'};this.submitting=false;this.persistPendingDraft();this.$message.error(this.pendingDraft.error)}},
