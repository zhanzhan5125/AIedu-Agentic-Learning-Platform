<template>
  <div class="page-shell" :class="{ 'student-page': !isTeacher }" v-loading="loading">
    <div class="page-heading"><div><h2>{{ isTeacher ? '班级学情洞察' : '我的学习画像' }}</h2><p>{{ isTeacher ? '展示已发布课程路线的全部知识点；每次确认批阅后，平均掌握度会根据得分自动更新。' : '掌握度按已评分作业 70% 与已评分诊断练习 30% 加权计算。' }}</p></div><div v-if="!isTeacher" class="heading-actions"><el-button v-if="(practice.questions || []).length" @click="practiceVisible=true">查看最近练习</el-button><el-button type="primary" :loading="generating" @click="generatePractice">{{ practiceButtonText }}</el-button></div></div>
...
export default {
  data() { return { loading: false, data: {}, generating: false, practiceVisible: false, practice: {}, practiceRunId: null, evidenceVisible: false, evidencePoint: {} } },
  computed: {
  ...
  created() { this.load() },
  methods: {
    async load() { if (!this.offeringId) return; this.loading = true; try { const path = this.isTeacher ? `/teacher/offerings/${this.offeringId}/insights` : `/student/offerings/${this.offeringId}/learning-profile`; this.data = (await apiV1.get(path)).data || {}; if(!this.isTeacher) await this.loadLatestPractice() } catch (_) { this.$message.error('学情数据加载失败') } finally { this.loading = false } },
    async loadLatestPractice() { const latest=(await apiV1.get(`/student/offerings/${this.offeringId}/practice-runs/latest`)).data; if(latest){this.practiceRunId=latest.id;if(latest.status==='succeeded'&&latest.result)this.practice=latest.result} },
    ...
    async generatePractice(){
      ...
      try{
        const key=`practice-${this.offeringId}-${Date.now()}`
        const created=await apiV1.post(`/student/offerings/${this.offeringId}/practice-jobs`,{idempotency_key:key,knowledge_point_ids:[],question_count:5})
        this.practiceRunId=created.data.agent_run_id
        for(let i=0;i<240;i++){
          const run=(await apiV1.get(`/agent-runs/${this.practiceRunId}`)).data
          if(run.status==='succeeded'){
            this.practice=run.result||{}
            this.practiceVisible=true
            return
          }
          if(run.status==='failed'||run.status==='cancelled') throw new Error(run.error||run.last_error||'练习生成失败')
          await new Promise(resolve=>setTimeout(resolve,1000))
        }
        throw new Error('生成时间较长，任务仍可能在后台继续；稍后返回本页即可查看最近练习')
      }catch(error){
        this.$message.error(this.errorMessage(error)||'个性化练习生成失败')
      }finally{
