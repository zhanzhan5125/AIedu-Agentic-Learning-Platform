<template>
  <div class="page-shell" :class="{ 'student-page': !isTeacher }" v-loading="loading">
    <div class="page-heading"><div><h2>{{ isTeacher ? '班级学情洞察' : '我的学习画像' }}</h2><p>{{ isTeacher ? '活跃度与知识掌握度分开计算，所有结论均可追溯到学习证据。' : '掌握度按已评分作业 70% 与已评分诊断练习 30% 加权计算。' }}</p></div><el-button v-if="!isTeacher" type="primary" :loading="generating" @click="generatePractice">{{ practiceButtonText }}</el-button></div>
