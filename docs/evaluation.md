...
`Aiedu-student-study FastAPI后端/evals/v1` 当前包含完整候选规模：

| 数据集 | 数量 | 当前状态 |
|---|---:|---|
| RAG | 40 条，其中 32 条可回答、8 条不可回答 | 待人工确认 |
| 智能批阅 | 10 份合成提交、30 条答案 | 待人工确认 |
| 意图路由 | 40 条，其中 10 条重复运行 | 待人工确认 |

候选数据统一使用 `approved: false`。正式命令会拒绝未审核数据，防止把机器生成标签包装成人工金标准。
...
在正式数据全部确认并完成 RAG 端到端人工盲审前，不填写提升数字。正式报告生成后，只能引用 JSON 中能够重新计算的指标；如果 Rerank 或 Reflection 没有提升，则保留消融结果并说明最终取舍。
